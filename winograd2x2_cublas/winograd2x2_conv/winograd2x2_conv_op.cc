...
REGISTER_OP("Winograd2x2Conv")
    ...
    ::tensorflow::shape_inference::DimensionHandle B = c->Dim(c->input(0),1);
    ::tensorflow::shape_inference::DimensionHandle nH = c->Dim(c->input(0),2);
    ::tensorflow::shape_inference::DimensionHandle nW = c->Dim(c->input(0),3);
    ::tensorflow::shape_inference::DimensionHandle K = c->Dim(c->input(1),2);
    ::tensorflow::shape_inference::DimensionHandle H;
    ::tensorflow::shape_inference::DimensionHandle W;
    TF_RETURN_IF_ERROR(c->Multiply(nH,2,&H));
    TF_RETURN_IF_ERROR(c->Multiply(nW,2,&W));
    const std::vector<::tensorflow::shape_inference::DimensionHandle> dims = {B,H,W,K};
