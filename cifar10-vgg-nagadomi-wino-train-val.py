...
class Model(ModelDesc):
    ...
    def _build_graph(self, inputs):
        ...
        # TODO:VGG-Nagadomi-Winograd-Relu Network
        with argscope(Conv2D, nl=tf.identity, use_bias=False,
                      W_init=variance_scaling_initializer(mode='FAN_OUT')), \
                argscope([Conv2D, MaxPooling, GlobalAvgPooling, BatchNorm], data_format=self.data_format):
            # VGG-Nagadomi-Winograd-Relu
            l = WinogradImTrans('WinogradImTrans_0',image,tf.nn.relu)
            l = WinogradConv('WinogradConv_0',l,3,64,mask=mask_dict['WinogradConv_0/W'] if use_mask else None)
            l = WinogradImTrans('WinogradImTrans_1',image,tf.nn.relu)
            l = WinogradConv('WinogradConv_1',l,64,64,mask=mask_dict['WinogradConv_1/W'] if use_mask else None)
            l = MaxPooling('Pool0',l,2,stride=2,padding='SAME')
            l = Dropout('Drop_0',l,rate=0.25)
            l = WinogradImTrans('WinogradImTrans_2',l,tf.nn.relu)
            l = WinogradConv('WinogradConv_2',l,64,128,mask=mask_dict['WinogradConv_2/W'] if use_mask else None)
            l = WinogradImTrans('WinogradImTrans_3',l,tf.nn.relu)
            l = WinogradConv('WinogradConv_3',l,128,128,mask=mask_dict['WinogradConv_3/W'] if use_mask else None)
            l = MaxPooling('Pool1',l,2,stride=2,padding='SAME')
            l = Dropout('Drop_1',l,rate=0.25)
            l = WinogradImTrans('WinogradImTrans_4',l,tf.nn.relu)
            l = WinogradConv('WinogradConv_4',l,128,256,mask=mask_dict['WinogradConv_4/W'] if use_mask else None)
            l = WinogradImTrans('WinogradImTrans_5',l,tf.nn.relu)
            l = WinogradConv('WinogradConv_5',l,256,256,mask=mask_dict['WinogradConv_5/W'] if use_mask else None)
            l = WinogradImTrans('WinogradImTrans_6',l,tf.nn.relu)
            l = WinogradConv('WinogradConv_6',l,256,256,mask=mask_dict['WinogradConv_6/W'] if use_mask else None)
            l = WinogradImTrans('WinogradImTrans_7',l,tf.nn.relu)
            l = WinogradConv('WinogradConv_7',l,256,256,mask=mask_dict['WinogradConv_7/W'] if use_mask else None)
            l = MaxPooling('Pool2',l,2,stride=2,padding='SAME')
            l = Dropout('Drop2',l,rate=0.25)
            l = FullyConnected('Linear0',l,1024,nl=tf.identity)
            l = tf.nn.relu(l)
            l = Dropout('Drop_fc0',l,rate=0.5)
            l = FullyConnected('Linear1',l,1024,nl=tf.identity)
            l = tf.nn.relu(l)
            l = Dropout('Drop_fc1',l,rate=0.5)
            logits = FullyConnected('Linear2',l,10,nl=tf.identity)
            tf.nn.softmax(logits,name='output')
        # define cost function here
        cost = tf.nn.sparse_softmax_cross_entropy_with_logits(logits = logits,labels=label)
        cost = tf.reduce_mean(cost,name='corss_entropy_loss')
        # compute error
        wrong = tf.to_float(tf.logical_not(tf.nn.in_top_k(logits,label,1)),name='wrong_vector')
        # weight decay on all W of fc layers
        # wd_w = 0.0002 * 0.2 ^ (get_global_step_var / 480000)
        # TODO:do we need this weight decay of FC layers?
...
def eval_on_CIFAR10(model_file,data_dir):
    ds = get_data('test')
