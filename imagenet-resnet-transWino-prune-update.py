...
sys.path.append('/home/yangtao/sparse-winograd-cnn/Resnet-18-var')
...
def get_config(fake=False, data_format='NHWC'):
    if fake:
        dataset_train = dataset_val = FakeData(
            [[64, 224, 224, 3], [64]], 10, random=False, dtype='uint8')
    else:
    ...
    return TrainConfig(
        ...
        steps_per_epoch=1280000 // TOTAL_BATCH_SIZE // eval_freq if not test else 1280000//eval_freq,
        # steps_per_epoch=0,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--resume', help='TF checkpoint file')
    ...
    parser.add_argument('--val',help='validate the model')
    parser.add_argument('--start_epoch',type=int,help='starting epoch number')
    ...
    if args.val:
        ...
        if use_mask:
            ...
            for k,v in mask_dict.items():
                mask_dict[k] = v.astype(np.float32)
        ...
        if args.resume:
            config.session_init = SaverRestore(args.resume)
        if args.start_epoch:
            config.starting_epoch = args.start_epoch
        if use_mask:
            mask_dict = pickle.load(open(mask_file_dir, 'rb'))
            # apply type cast
            for k,v in mask_dict.items():
